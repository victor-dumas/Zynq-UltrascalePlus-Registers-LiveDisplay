...
var mySessionStorage = (function() {
	...
	var result;
	try {
		//(storage = window.localStorage).setItem(uid, uid);  //local - setting kept no expiration
        (storage = window.sessionStorage).setItem(uid, uid);  //session - setting lost wen browser closes
...
function storage_SetItem(key, val, fLocalStorage) {  //defaul is session storage
    if (myLocalStorage && mySessionStorage)
    {
        if (fLocalStorage) myLocalStorage.setItem(key, val);
        else mySessionStorage.setItem(key, val);
    }
}
...
function storage_GetItem(key, defaultVal, fLocalStorage) {  //defaul is session storage
    if (myLocalStorage && mySessionStorage) {
        var ret;
        if (fLocalStorage) ret = myLocalStorage.getItem(key); 
        else ret = mySessionStorage.getItem(key); 
        if (ret !== null)
            return ret;
    }
...
function storage_RemoveItem(key, fLocalStorage) {
    if (myLocalStorage && mySessionStorage) {
        if (fLocalStorage) myLocalStorage.removeItem(key); 
        else mySessionStorage.removeItem(key); 
    }
}
...
function GetRightPage() {
    ...
    else if (mySessionStorage) {   // HTML5 sessionStorage available?
        if (!RightFrameFound_AtLeastOnce) {
        	var rightPath = mySessionStorage.getItem("hw_rightPath");  // item set by NavScript.js
        	if (rightPath)
                return rightPath;
        }	
    }
