function reqListener(req, e) {
    ...
    if (req.status == 200) {
        ...
        Array.prototype.forEach.call(values, node => {
            //var value = response[value.childNodes[0].nodeValue];
            Object.keys(response).forEach(addr => {
                if (addr == node.id) {
                    //console.log(addr + ':' + response[addr])
...
function getValues() {
    ...
    req.addEventListener("load", reqListener.bind(null, req), false);
